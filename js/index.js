var model = {
  ...
  getData: function(responseText) {
    ...
    function degToCompass(deg) {
      ...
      var index = Math.floor((deg % 360) / dirStep + 0.5) % 16;
...
var view = {
  ...
  updateWeather: function() {
    ...
    locationName.innerHTML =
      "" + model.data.city + ", " + model.data.country.toUpperCase() + "";
    // Current weather icon
    var weatherIcon = document.getElementById("curr-weather-icon");
    weatherIcon.setAttribute(
      "class",
      "wi wi-owm-" + isDayOrNight() + "-" + model.data.weatherID + ""
    );
    ...
    currWindData.innerHTML =
      model.data.windDir +
      " " +
      model.data["windSpeed" + model.currUnits] +
      (model.currUnits === "Metric" ? " km/h" : " mph");
    currWindIcon.setAttribute(
      "class",
      "wi wi-wind wi-from-" + model.data.windDir.toLowerCase() + ""
    );
  ...
  getUnits: function() {
    // Get selected units
    view.switchUnitsBtn.checked
      ? (model.currUnits = "Imp")
      : (model.currUnits = "Metric");
...
var controller = {
  getGeoLocation: function() {
    ...
    function displayError(error) {
      ...
      switch (error.code) {
        ...
        case 2:
          errorMessage =
            "We can't detect your position. Try reloading the page or manually type your location in the search bar above.";
          break;
        case 3:
        ...
        default:
          errorMessage =
            "Looks like we're having some trouble right now. Try a refresh.";
      }
  ...
  setAPIcall: function(APIcallMethod, a, b) {
    ...
    url += "&appid=d14a0ad3044917059665e512dfb9d5eb";
    // Set the request
